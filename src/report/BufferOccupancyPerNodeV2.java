...
public class BufferOccupancyPerNodeV2 extends Report implements UpdateListener {
	/**
	...
	public static final int DEFAULT_BUFFER_REPORT_INTERVAL = 300;// 3600

	private double lastRecord = Double.MIN_VALUE;
	private int interval;

	private Map<DTNHost, List<String>> bufferPerNode;

	private Map<DTNHost, Double> bufferCounts = new HashMap<DTNHost, Double>();
	private int updateCounter = 0; // new added

	public BufferOccupancyPerNodeV2() {
		super();
		this.bufferPerNode = new HashMap<DTNHost, List<String>>();
		...
		if (SimClock.getTime() - lastRecord >= interval) {
			...
			for (DTNHost host : SimScenario.getInstance().getHosts()) {
				if (this.bufferPerNode.containsKey(host)) {
					List<String> bufferList = this.bufferPerNode.get(host);
					bufferList.add(format(host.getBufferOccupancy()));
