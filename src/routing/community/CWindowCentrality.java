 ...
 * @see Centrality
 */
public class CWindowCentrality implements CentralityKu {
	...
	@Override
	public int[] getArrayCentrality(Map<DTNHost, List<Duration>> connHistory) {
		if (SimClock.getIntTime() - this.lastGlobalComputationTime < COMPUTE_INTERVAL)
			return null; // atau return array terakhir jika ingin menyimpannya
		...
		for (Map.Entry<DTNHost, List<Duration>> entry : connHistory.entrySet()) {
			...
			for (Duration d : entry.getValue()) {
				...
				if (epoch >= EPOCH_COUNT) // prevent ArrayIndexOutOfBounds
					continue;
